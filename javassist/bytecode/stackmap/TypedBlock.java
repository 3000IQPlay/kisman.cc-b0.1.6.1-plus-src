...
public class TypedBlock extends BasicBlock
{
    ...
    void initFirstBlock(final int maxStack, final int maxLocals, final String className, final String methodDesc, final boolean isStatic, final boolean isConstructor) throws BadBytecode {
        ...
        if (isConstructor) {
            locals[0] = (TypeData)new TypeData.UninitThis(className);
        }
        else if (!isStatic) {
            locals[0] = (TypeData)new TypeData.ClassName(className);
        }
        int n = isStatic ? -1 : 0;
        int i = 1;
        try {
    ...
    private static int descToTag(final String desc, int i, final int n, final TypeData[] types) throws BadBytecode {
        ...
        if (c == 'L') {
            ...
            if (arrayDim > 0) {
                types[n] = (TypeData)new TypeData.ClassName(desc.substring(i2, ++i3));
            }
            else {
                types[n] = (TypeData)new TypeData.ClassName(desc.substring(i2 + 1, ++i3 - 1).replace('/', '.'));
            }
            return i3;
        }
        if (arrayDim > 0) {
            types[n] = (TypeData)new TypeData.ClassName(desc.substring(i2, ++i));
            return i;
        }
    ...
    private static TypeData toPrimitiveTag(final char c) {
        switch (c) {
            ...
            case 'S':
            case 'Z': {
                return (TypeData)TypeTag.INTEGER;
            }
            case 'J': {
                return (TypeData)TypeTag.LONG;
            }
            case 'F': {
                return (TypeData)TypeTag.FLOAT;
            }
            case 'D': {
                return (TypeData)TypeTag.DOUBLE;
            }
            default: {
                return null;
            }
