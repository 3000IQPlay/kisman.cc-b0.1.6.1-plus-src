...
public class ExprEditor
{
    public boolean doit(final CtClass clazz, final MethodInfo minfo) throws CannotCompileException {
        ...
        try {
            ...
            throw new CannotCompileException(b.getMessage(), (Throwable)b);
        }
        return edited;
    }
        ...
        try {
            ...
            if (c >= 178) {
                if (c < 188) {
                    if (c == 184 || c == 185 || c == 182) {
                        ...
                        if (newList != null && minfo.getConstPool().isConstructor(newList.type, iterator.u16bitAt(pos + 1)) > 0) {
                            ...
                            if (mcall.getMethodName().equals("<init>")) {
                                final ConstructorCall ccall = (ConstructorCall)(expr = (Expr)new ConstructorCall(pos, iterator, clazz, minfo));
                                this.edit(ccall);
                            }
                            else {
                                expr = mcall;
                    ...
                    expr = (Expr)new Cast(pos, iterator, clazz, minfo);
                    this.edit((Cast)expr);
                }
            }
