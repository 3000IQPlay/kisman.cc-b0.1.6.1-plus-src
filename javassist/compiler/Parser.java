...
public final class Parser implements TokenId
{
    ...
    public ASTList parseMember(final SymbolTable tbl) throws CompileError {
        ...
        if (mem instanceof MethodDecl) {
            return (ASTList)this.parseMethod2(tbl, (MethodDecl)mem);
        }
        return mem;
    }
    ...
    public ASTList parseMember1(final SymbolTable tbl) throws CompileError {
        ...
        if (isConstructor || this.lex.lookAhead() == 40) {
            return (ASTList)this.parseMethod1(tbl, isConstructor, mods, d);
        }
        return (ASTList)this.parseField(tbl, mods, d);
    }
    ...
    private MethodDecl parseMethod1(final SymbolTable tbl, final boolean isConstructor, final ASTList mods, final Declarator d) throws CompileError {
        ...
        if (this.lex.lookAhead() != 41) {
            while (true) {
                parms = ASTList.append(parms, (ASTree)this.parseFormalParam(tbl));
        ...
        if (this.lex.lookAhead() == 341) {
            this.lex.get();
            while (true) {
                throwsList = ASTList.append(throwsList, (ASTree)this.parseClassType(tbl));
        ...
        return new MethodDecl((ASTree)mods, new ASTList((ASTree)d, ASTList.make((ASTree)parms, (ASTree)throwsList, (ASTree)null)));
    }
    ...
    private Stmnt parseDeclarators(final SymbolTable tbl, final Declarator d) throws CompileError {
        Stmnt decl = null;
        while (true) {
            decl = (Stmnt)ASTList.concat((ASTList)decl, (ASTList)new Stmnt(68, (ASTree)this.parseDeclarator(tbl, d)));
            final int t = this.lex.get();
            if (t == 59) {
                return decl;
            }
    ...
    private ASTree parseInitializer(final SymbolTable tbl) throws CompileError {
        if (this.lex.lookAhead() == 123) {
            return (ASTree)this.parseArrayInitializer(tbl);
        }
    ...
    private ASTree parseInstanceOf(final SymbolTable tbl, final ASTree expr) throws CompileError {
        ...
        if (isBuiltinType(t)) {
            this.lex.get();
            final int dim = this.parseArrayDimension();
            return (ASTree)new InstanceOfExpr(t, dim, expr);
        }
        final ASTList name = this.parseClassType(tbl);
        final int dim2 = this.parseArrayDimension();
        return (ASTree)new InstanceOfExpr(name, dim2, expr);
    }
    ...
    private ASTree parseUnaryExpr(final SymbolTable tbl) throws CompileError {
        switch (this.lex.lookAhead()) {
            ...
            case 363: {
                ...
                return (ASTree)Expr.make(t, this.parseUnaryExpr(tbl));
            }
            case 40: {
    ...
    private ASTree parseCast(final SymbolTable tbl) throws CompileError {
        ...
        if (isBuiltinType(t) && this.nextIsBuiltinCast()) {
            ...
            return (ASTree)new CastExpr(t, dim, this.parseUnaryExpr(tbl));
        }
        else {
            ...
            return (ASTree)new CastExpr(name, dim2, this.parseUnaryExpr(tbl));
        }
    }
                    ...
                    switch (this.lex.lookAhead()) {
                        ...
                        case 91: {
                            if (this.lex.lookAhead(1) == 93) {
                                ...
                                expr = (ASTree)Expr.make(65, expr, index);
                                continue;
                            }
                            break;
                        }
                        ...
                        case 363: {
                            final int t = this.lex.get();
                            expr = (ASTree)Expr.make(t, (ASTree)null, expr);
                            continue;
                        }
                        case 46: {
                            this.lex.get();
                            ...
                            if (t == 336) {
                                expr = (ASTree)Expr.make(46, (ASTree)new Symbol(this.toClassName(expr)), (ASTree)new Keyword(t));
                                continue;
                            }
                            if (t == 400) {
                                final String str = this.lex.getString();
                                expr = (ASTree)Expr.make(46, expr, (ASTree)new Member(str));
                                continue;
                            }
                        ...
                        case 35: {
                            ...
                            final String str = this.lex.getString();
                            expr = (ASTree)Expr.make(35, (ASTree)new Symbol(this.toClassName(expr)), (ASTree)new Member(str));
                            continue;
                        }
                        default: {
                            return expr;
    ...
    private ASTree parseMethodCall(final SymbolTable tbl, final ASTree expr) throws CompileError {
        ...
        return (ASTree)CallExpr.makeCall(expr, (ASTree)this.parseArgumentList(tbl));
    }
    ...
    private ASTree parsePrimaryExpr(final SymbolTable tbl) throws CompileError {
        ...
        switch (t = this.lex.get()) {
            ...
            case 328: {
                return (ASTree)this.parseNew(tbl);
            }
            case 40: {
                final ASTree expr = this.parseExpression(tbl);
                if (this.lex.get() == 41) {
