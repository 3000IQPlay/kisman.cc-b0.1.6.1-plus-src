...
public class TypeChecker extends Visitor implements Opcode, TokenId
{
    ...
    public void atNewArrayExpr(final NewExpr expr) throws CompileError {
        ...
        final ASTree init = (ASTree)expr.getInitializer();
        if (init != null) {
    ...
    public void atArrayInit(final ArrayInit init) throws CompileError {
        ASTList list = (ASTList)init;
        while (list != null) {
            final ASTree h = list.head();
            list = list.tail();
            if (h != null) {
                h.accept((Visitor)this);
            }
        }
    }
    ...
    public void atAssignExpr(final AssignExpr expr) throws CompileError {
        ...
        if (left instanceof Variable) {
            this.atVariableAssign((Expr)expr, op, (Variable)left, ((Variable)left).getDeclarator(), right);
        }
        else {
            if (left instanceof Expr) {
                final Expr e = (Expr)left;
                if (e.getOperator() == 65) {
                    this.atArrayAssign((Expr)expr, op, (Expr)left, right);
                    return;
                }
            }
            this.atFieldAssign((Expr)expr, op, left, right);
        }
    }
    ...
    public void atBinExpr(final BinExpr expr) throws CompileError {
        ...
        if (k >= 0) {
            if (token == 43) {
                ...
                if (e != null) {
                    e = (Expr)CallExpr.makeCall((ASTree)Expr.make(46, (ASTree)e, (ASTree)new Member("toString")), (ASTree)null);
                    expr.setOprand1((ASTree)e);
    ...
    private Expr atPlusExpr(final BinExpr expr) throws CompileError {
        ...
        if ((type1 == 307 && dim1 == 0 && "java/lang/String".equals(cname)) || (this.exprType == 307 && this.arrayDim == 0 && "java/lang/String".equals(this.className))) {
            final ASTList sbufClass = ASTList.make((ASTree)new Symbol("java"), (ASTree)new Symbol("lang"), (ASTree)new Symbol("StringBuffer"));
            final ASTree e = (ASTree)new NewExpr(sbufClass, (ASTList)null);
            this.exprType = 307;
            this.arrayDim = 0;
            this.className = "java/lang/StringBuffer";
            return makeAppendCall((ASTree)makeAppendCall(e, left), right);
        }
    ...
    private boolean isConstant(final BinExpr expr, final int op, ASTree left, ASTree right) throws CompileError {
        ...
        if (left instanceof StringL && right instanceof StringL && op == 43) {
            newExpr = (ASTree)new StringL(((StringL)left).get() + ((StringL)right).get());
        }
    ...
    public static ASTree getConstantFieldValue(final CtField f) {
        ...
        if (value instanceof Boolean) {
            return (ASTree)new Keyword(((boolean)value) ? 410 : 411);
        }
        return null;
    }
    ...
    private void booleanExpr(final ASTree expr) throws CompileError {
        ...
        if (op == 358) {
            final BinExpr bexpr = (BinExpr)expr;
            bexpr.oprand1().accept((Visitor)this);
            final int type1 = this.exprType;
            final int dim1 = this.arrayDim;
            bexpr.oprand2().accept((Visitor)this);
            if (dim1 == 0 && this.arrayDim == 0) {
                this.insertCast(bexpr, type1, this.exprType);
            }
        }
        else if (op == 33) {
            ((Expr)expr).oprand1().accept((Visitor)this);
        }
    ...
    public void atCastExpr(final CastExpr expr) throws CompileError {
        ...
        expr.getOprand().accept((Visitor)this);
    ...
    protected CtField fieldAccess(final ASTree expr) throws CompileError {
        ...
        if (expr instanceof Expr) {
            ...
            if (op == 35) {
                ...
                final CtField f2 = this.resolver.lookupField(((Symbol)e.oprand1()).get(), (Symbol)mem2);
                mem2.setField(f2);
                return f2;
            }
            if (op == 46) {
                try {
                    e.oprand1().accept((Visitor)this);
                }
                catch (NoFieldException nfe) {
    ...
    private CtField fieldAccess2(final Expr e, final String jvmClassName) throws CompileError {
        ...
        final CtField f = this.resolver.lookupFieldByJvmName2(jvmClassName, (Symbol)fname, (ASTree)e);
        e.setOperator(35);
    ...
    public void atArrayLength(final Expr expr) throws CompileError {
        expr.oprand1().accept((Visitor)this);
        if (this.arrayDim == 0) {
            throw new NoFieldException("length", (ASTree)expr);
        }
        this.exprType = 324;
        this.arrayDim = 0;
    }
