...
public class ParserImpl implements Parser
{
    ...
    private Event parseNode(final boolean block, final boolean indentlessSequence) {
        ...
        if (this.scanner.checkToken(Token.ID.Alias)) {
            ...
            if (indentlessSequence && this.scanner.checkToken(Token.ID.BlockEntry)) {
                ...
                event = (Event)new SequenceStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.FALSE);
                ...
                event = (Event)new ScalarEvent(anchor, tag, implicitValues, token4.getValue(), startMark, endMark, Character.valueOf(token4.getStyle()));
                this.state = this.states.pop();
            }
            else if (this.scanner.checkToken(Token.ID.FlowSequenceStart)) {
                endMark = this.scanner.peekToken().getEndMark();
                event = (Event)new SequenceStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.TRUE);
                this.state = new ParseFlowSequenceFirstEntry();
            }
            else if (this.scanner.checkToken(Token.ID.FlowMappingStart)) {
                endMark = this.scanner.peekToken().getEndMark();
                event = (Event)new MappingStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.TRUE);
                ...
                endMark = this.scanner.peekToken().getStartMark();
                event = (Event)new MappingStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.FALSE);
                ...
                event = (Event)new ScalarEvent(anchor, tag, new ImplicitTuple(implicit, false), "", startMark, endMark, Character.valueOf('\0'));
                this.state = this.states.pop();
            }
        }
        return event;
    ...
    private Event processEmptyScalar(final Mark mark) {
        return (Event)new ScalarEvent((String)null, (String)null, new ImplicitTuple(true, false), "", mark, mark, Character.valueOf('\0'));
    }
    ...
    private class ParseStreamStart implements Production
    {
        ...
        public Event produce() {
            ...
            final Event event = (Event)new StreamStartEvent(token.getStartMark(), token.getEndMark());
    ...
    private class ParseImplicitDocumentStart implements Production
    {
        ...
        public Event produce() {
            if (!ParserImpl.this.scanner.checkToken(Token.ID.Directive, Token.ID.DocumentStart, Token.ID.StreamEnd)) {
                ...
                final Event event = (Event)new DocumentStartEvent(startMark, endMark, false, (DumperOptions.Version)null, (Map)null);
    ...
    private class ParseDocumentStart implements Production
    {
        ...
        public Event produce() {
            ...
            if (!ParserImpl.this.scanner.checkToken(Token.ID.StreamEnd)) {
                ...
                event = (Event)new DocumentStartEvent(startMark, endMark, true, tuple.getVersion(), (Map)tuple.getTags());
                ParserImpl.this.states.push(new ParseDocumentEnd());
                ...
                event = (Event)new StreamEndEvent(token2.getStartMark(), token2.getEndMark());
    ...
    private class ParseDocumentEnd implements Production
    {
        ...
        public Event produce() {
            ...
            final Event event = (Event)new DocumentEndEvent(startMark, endMark, explicit);
    ...
    private class ParseBlockSequenceEntry implements Production
    {
        ...
        public Event produce() {
            if (ParserImpl.this.scanner.checkToken(Token.ID.BlockEntry)) {
                ...
                final Event event = (Event)new SequenceEndEvent(token2.getStartMark(), token2.getEndMark());
                ParserImpl.this.state = ParserImpl.this.states.pop();
    ...
    private class ParseIndentlessSequenceEntry implements Production
    {
        ...
        public Event produce() {
            if (!ParserImpl.this.scanner.checkToken(Token.ID.BlockEntry)) {
                final Token token = ParserImpl.this.scanner.peekToken();
                final Event event = (Event)new SequenceEndEvent(token.getStartMark(), token.getEndMark());
                ParserImpl.this.state = ParserImpl.this.states.pop();
                return event;
            }
            final Token token = ParserImpl.this.scanner.getToken();
    ...
    private class ParseBlockMappingKey implements Production
    {
        ...
        public Event produce() {
            if (ParserImpl.this.scanner.checkToken(Token.ID.Key)) {
                ...
                final Token token = ParserImpl.this.scanner.getToken();
                final Event event = (Event)new MappingEndEvent(token.getStartMark(), token.getEndMark());
                ParserImpl.this.state = ParserImpl.this.states.pop();
                ParserImpl.this.marks.pop();
                return event;
            }
    ...
    private class ParseFlowSequenceEntry implements Production
    {
        ...
        public Event produce() {
            if (!ParserImpl.this.scanner.checkToken(Token.ID.FlowSequenceEnd)) {
                ...
                if (ParserImpl.this.scanner.checkToken(Token.ID.Key)) {
                    final Token token = ParserImpl.this.scanner.peekToken();
                    final Event event = (Event)new MappingStartEvent((String)null, (String)null, true, token.getStartMark(), token.getEndMark(), Boolean.TRUE);
