...
public class Yaml
{
    ...
    public Yaml(final DumperOptions dumperOptions) {
        this((BaseConstructor)new Constructor(), new Representer(), dumperOptions);
    }
    ...
    private void dumpAll(final Iterator<?> data, final Writer output, final Tag rootTag) {
        final Serializer serializer = new Serializer((Emitable)new Emitter(output, this.dumperOptions), this.resolver, this.dumperOptions, rootTag);
        try {
            serializer.open();
            while (data.hasNext()) {
                final Node node = this.representer.represent((Object)data.next());
    ...
    public List<Event> serialize(final Node data) {
        ...
        final Serializer serializer = new Serializer((Emitable)emitter, this.resolver, this.dumperOptions, (Tag)null);
        try {
            serializer.open();
    ...
    private Object loadFromReader(final StreamReader sreader, final Class<?> type) {
        final Composer composer = new Composer((Parser)new ParserImpl(sreader), this.resolver);
        this.constructor.setComposer(composer);
        return this.constructor.getSingleData((Class)type);
    }
    
    public Iterable<Object> loadAll(final Reader yaml) {
        final Composer composer = new Composer((Parser)new ParserImpl(new StreamReader(yaml)), this.resolver);
        this.constructor.setComposer(composer);
    ...
    public Iterable<Event> parse(final Reader yaml) {
        final Parser parser = (Parser)new ParserImpl(new StreamReader(yaml));
